...
def speeds(G, API_KEY):
    ...
    x2 = str(nodes['x'].max())
    print(y1, x1, y2, x2)
    url = 'https://data.traffic.hereapi.com/v7/flow?apiKey={}&in=bbox:{},{},{},{}&locationReferencing=shape'.format(API_KEY, x1, y2, x2, y1)
    page = requests.get(url)
    jsonn = json.loads(page.text)
    roads = jsonn['results']
    ...
    c = 0
    for road in roads:
        su = road['currentFlow']['speedUncapped']
        ff = road['currentFlow']['freeFlow']
        shps = road['location']['shape']['links']
        for j in range(0, len(shps)):
            latlong = shps[j]['points']
            # loc_list=[]
            la = []
            lo = []
            su1 = []
            ...
            for i in range(0, int(len(latlong))):
                la.append(float(latlong[i]['lat']))
                lo.append(float(latlong[i]['lng']))
                su1.append(float(su))
                ff1.append(float(ff))
            lats.append(la)
            longs.append(lo)
...
def add_edge_speeds3(G, edges,hwy_speeds=None, fallback=None, precision=1, agg=np.mean):
    ...
    if "speed" in edges.columns:
        ...
        # kph if necessary
        edges["speed_kph"] = edges["speed"].astype(str).map(_clean_maxspeed).astype(float) * 3.6
    else:
